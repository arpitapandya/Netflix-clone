...
describe('<SignUp />', () => {
  it('renders the sign up page with a form submission', async () => {
    ...
    await act(async () => {
      await fireEvent.change(getByPlaceholderText('First Name'), { target: { value: 'Arpita' } });
      await fireEvent.change(getByPlaceholderText('Email Address'), { target: { value: 'arpya74@gmail.com' } });
      await fireEvent.change(getByPlaceholderText('Password'), { target: { value: 'password' } });
      fireEvent.click(getByTestId('sign-up'));

      expect(getByPlaceholderText('Email Address').value).toBe('arpya74@gmail.com');
